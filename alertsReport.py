...
parser.add_argument('-o', '--outputfile', help='File name to wrtie report to (default=output.csv)', default='output.csv')
parser.add_argument('-p','--printoutput', help='Print output file to screen in addition to file', action='store_true')
parser.add_argument('-i', '--includeservers', help='Include results for New Relic Servers (soon to be deprecated)', action='store_true')
parser.add_argument('-lp', '--limitpolicies', help='Limit the number of policies returned', default=5000, type=int)
parser.add_argument('-lc', '--limitconditions', help='Limit the number of conditions returned per policy', default=250, type=int)
parser.add_argument('-id', '--ignoredisabled', help='Do not include disabled conditions in the report', action='store_true')
...
if response.status_code == requests.codes.ok:
	...
	for policy in policies_json['policies']:		
		...
		##Get the standard alert sconditions for this policy
		if policyCounter < limit_policies :
			if log_info: print('--Working on policy ' + str(policyCounter+1) + ' of ' + str(len(policies_json['policies'])) + ' with id ' + str(policy['id']) + ' named "' + policy['name'] + '"')
			...
			if response.status_code == requests.codes.ok:
				...
				if log_info: print ('----Number of standard conditions in policy id ' + str(policy['id']) + ': ' +str(len(conditions_json['conditions'])))
				...
				for condition in conditions_json['conditions']:
					if conditionCounter < limit_conditions:
						if log_info: print ('------Working on condition id ' + str(condition['id']) + ' named "' + condition['name'] + '" for policyId ' + str(policy['id']) + ' named "' + policy['name'] + '"')
						...
						for entity in condition['entities']:
							...
							if condition['type'] == 'apm_app_metric':
								...
								if response.status_code == requests.codes.ok:
									if log_debug: print('Success: Get Application api call for entity id ' + str(entity))
									servers_json = response.json()
							...
							lineToAdd = str(policy['id']) + ',' + policy['name'] + ',' + str(condition['id']) + ',' + condition['name'] + ',' + condition['type'] + ',' + str(condition['enabled']) + \
											',' + str(entity) + ',' + entityServerName + ',' + entityAppName + ',' + channels
			...
			if response.status_code == requests.codes.ok:
				...
				if log_info: print('----Number of synthetics conditions in policy ' + str(policy['id']) + ': ' + str(len(policies_syn['synthetics_conditions'])))

				for synCondition in policies_syn['synthetics_conditions']:
					lineToAdd = str(policy['id']) + ',' + policy['name'] + ',' + str(synCondition['id']) + ',' + synCondition['name'] + ',syn_check,' + str(synCondition['enabled']) + ',,,,' + channels + ','
					if ignore_disabled:
			...
			if response.status_code == requests.codes.ok:
				...
				if log_info: print('----Number of NRQL conditions in policy ' + str(policy['id']) + ': ' + str(len(policies_nrql['nrql_conditions'])))

				for nrqlCondition in policies_nrql['nrql_conditions']:
					lineToAdd = str(policy['id']) + ',' + policy['name'] + ',' + str(nrqlCondition['id']) + ',' + nrqlCondition['name'] + ',nrql,' + str(nrqlCondition['enabled']) + ',,,,' + channels + \
